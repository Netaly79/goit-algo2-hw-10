...
if __name__ == "__main__":
    ...
    for size, arr in test_arrays.items():
        ...
        for _ in range(5):
            arr_copy = arr[:]
            deterministic_times.append(measure_exec_time(deterministic_quick_sort, arr_copy))
        avg_deterministic_time = sum(deterministic_times) / len(deterministic_times)
        deterministic_results.append((size, avg_deterministic_time))
        print(f"Детермінований QuickSort: середній час {avg_deterministic_time:.4f} секунд")

        # Рандомізований QuickSort
        randomized_times = []
        for _ in range(5):
            arr_copy = arr[:]
            randomized_times.append(measure_exec_time(randomized_quick_sort, arr_copy))
        avg_randomized_time = sum(randomized_times) / len(randomized_times)
        randomized_results.append((size, avg_randomized_time))
        print(f"Рандомізований QuickSort: середній час {avg_randomized_time:.4f} секунд")
